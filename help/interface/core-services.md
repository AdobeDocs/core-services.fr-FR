---
description: Modernisez vos applications Adobe Analytics et Adobe Target pour les services interapplications. Découvrez comment débuter avec les services Experience Cloud.
keywords: services principaux;attributs du client
solution: Experience Cloud
title: Activation des applications pour les services inter-applications
index: true
...
Les clients existants peuvent moderniser leurs implémentations d’application et mettre en oeuvre Experience Cloud. Cela vous permet d’utiliser les attributs du client et les fonctionnalités d’audience dans Adobe Analytics, Audience Manager et Adobe Target. Suivez les étapes suivantes afin de réaliser cette mise en œuvre :
...
* appartenir à un groupe d’applications mappé à un groupe d’entreprises.
* Si nécessaire, liez leurs comptes d’application à leur Adobe ID (comme décrit ci-dessous).

### Facultatif : liez les comptes d’utilisateurs existants.

Il est probable que des utilisateurs soient déjà membres de groupes d’applications, tels qu’un groupe Analytics dans lequel vous avez précédemment géré [!UICONTROL Analytics] > [!UICONTROL Outils d’administration].

Lorsque vous mappez ces groupes avec des groupes d’entreprise Experience Cloud, ces utilisateurs doivent associer manuellement les informations d’identification de leur compte d’application à leur Adobe ID.
...
>[!NOTE]
>
>Une fois les groupes d’entreprise et d’application mappés, les nouveaux utilisateurs sont automatiquement liés. (Les informations de connexion de la solution sont automatiquement créées et liées à leur Adobe ID.)
...
Le [!UICONTROL Service d’ID Experience Cloud] fournit un identifiant commun pour l’intégration inter-applications. Il offre une identification des visiteurs interdomaines ainsi qu’un chemin d’accès pour le ciblage et la personnalisation interpériphérique/des navigateurs basés sur les données de gestion de la relation client transférées par le biais d’[!UICONTROL Attributs du client].
...
* Il est recommandé d’ajouter une [extension Adobe Target](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/target-v2/overview.html?lang=fr) dans [!UICONTROL Experience Platform Launch] afin de permettre l’extraction automatique de votre bibliothèque. Vous pouvez également configurer la variable [Extension du service d’ID Experience Cloud](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=en) pour Adobe Target (et d’autres applications) à l’aide de [!UICONTROL Experience Platform Launch]. Le [!UICONTROL service Experience Cloud ID] **doit être mis à jour** pour qu’Adobe Target puisse utiliser les services principaux.
...
* Demandez l’accès afin d’utiliser Adobe Analytics comme source de création de rapports pour [!DNL Adobe Target]. [!DNL Target] et [!DNL Analytics] Les données sont combinées dans le même appel serveur lors du traitement afin que les visiteurs soient connectés entre les deux applications. Voir [Implémentation d’Analytics for Target](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t.html?lang=fr).
...
![Réponse attendue contenant l’identifiant Experience Cloud](assets/mac_id_response.png)

Requête d’image Analytics contenant l’Experience Cloud ID (également appelé `mid` ou _identifiant visiteur_) :

![Demande d’image Analytics contenant l’identifiant Experience Cloud](assets/mid.png)

Experience Cloud ID dans la requête de mbox :

![Identifiant Experience Cloud dans la requête de mbox](assets/mbox_request.png)
...
![Admin Console d’accès](assets/menu-administration-shell.png)
