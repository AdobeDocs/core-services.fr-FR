---
description: Découvrez les exigences en matière de fichiers de données et de sources de données multiples pour le chargement [!DNL Customer Attributes] à l'Experience Cloud.
...
Exigences relatives aux fichiers de données et sources de données multiples pour le chargement [!DNL Customer Attributes] à l&#39;Experience Cloud.

Il vous faut accéder à la gestion de la relation client (CRM) ou à d’autres données similaires de votre société. Les données que vous chargez vers l’Experience Cloud doivent être `.csv` fichier . Si vous chargez le fichier via FTP ou sFTP, vous devez également charger un fichier `.fin`.
