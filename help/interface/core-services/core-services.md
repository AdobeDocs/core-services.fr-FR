---
description: Mettez en œuvre Experience Cloud et devenez administrateur. Ce processus modernise vos solutions pour les fonctionnalités de service principal, telles que les attributs du client et les  de .
keywords: core services;customer attributes
seo-description: Mettez en œuvre Experience Cloud et devenez administrateur. Ce processus modernise vos solutions pour les fonctionnalités de service principal, telles que les attributs du client et les  de .
...
Pour les clients existants, apprenez à moderniser vos implémentations de solution et à mettre en oeuvre Experience Cloud afin que vous puissiez utiliser des fonctionnalités telles que les attributs du client et les  de . Pour ce faire, vous allez :
...
1. [Association des suites de rapports à une organisation Experience Cloud](#section_7B08516B01BA421681DF03D0E86CE3BA)
1. [Mise à jour de votre code Analytics AppMeasurement](#section_1798D9D0F05C47E29816AC4EEB9A0913)
1. [Mise à jour de la mise en oeuvre de votre  Adobe](#section_C2F4493C7A36406DAE2266B429A4BD24)
...
1. Contactez votre gestionnaire de compte et  le processus de mise en service d’Experience Cloud.
...
Pour vous connecter à Experience Cloud, vos utilisateurs doivent :

1. Posséder un ID Adobe (ou Enterprise ID pour votre ).
1. Sign in at [experiencecloud.adobe.com](https://experiencecloud.adobe.com).
1. Appartient à un groupe de solutions mappé à un groupe d’entreprise.
...
Il est probable que vous ayez des utilisateurs qui sont déjà membres de groupes de solutions, tels qu’un groupe Analytics que vous avez précédemment géré dans [!UICONTROL Analytics] > Outils [!UICONTROL d’]administration.
...
Les sections suivantes décrivent comment moderniser votre implémentation. La modernisation de votre implémentation active les services principaux dans Experience Cloud.

## Étape 2. Mise en oeuvre du service [!UICONTROL d’ID] Experience Cloud à l’aide du lancement [!UICONTROL de la plateforme]Experience ou de la gestion  dynamique des balises {#section_3C9F6DF37C654D939625BB4D485E4354}

Le service [!UICONTROL d’ID] Experience Cloud fournit un ID commun pour l’intégration inter-solutions. Il fournit une identification des interdomaines et un chemin d’accès pour le ciblage et la personnalisation inter-périphériques/navigateurs basés sur les données de gestion de la relation client téléchargées via les attributs [!UICONTROL du]client.

La méthode la plus simple pour activer les services principaux d’Experience Cloud consiste à l’activer automatiquement pour Analytics et les Adobe via l’extension [du service d’ID](https://docs.adobe.com/content/help/en/launch/using/implement/solutions/idservice-save.html) d’expérience dans le lancement [!UICONTROL de la plateforme]d’expérience ou via l’outil ECID dans la gestion dynamique des balises. (Le lancement de la plateforme d’expérience est vivement recommandé.)

![](assets/menu-activation-shell.png)

Pour obtenir l’aide complète du service d’ID d’expérience Cloud (anciennement, ID de), [cliquez ici](https://docs.adobe.com/content/help/en/id-service/using/home.html).
...
| Tâche | Description |
| -----------| ---------- |  
| [Mise en œuvre du service Experience Cloud ID pour Analytics](https://docs.adobe.com/content/help/en/id-service/using/implementation/setup-analytics.html) | Adobe recommande également de définir d’autres ID [de client](https://docs.adobe.com/content/help/en/id-service/using/reference/authenticated-state.html). Ces identifiants sont associés à chaque et activent les fonctionnalités actuelles et futures dans Experience Cloud. |
| Mettez à jour le fichier [!DNL s_code] existant vers la version H.27.3 ou ultérieure ou le fichier [!DNL AppMeasurement.js] vers la version 1.4 ou ultérieure. | Ces fichiers peuvent être téléchargés dans le Gestionnaire de [code](https://docs.adobe.com/content/help/en/analytics/admin/admin-tools/code-manager-admin.html) des outils d’administration Analytics.  (Le guide de mise en oeuvre [](https://docs.adobe.com/content/help/en/analytics/implementation/javascript-implementation/javascript-implementation-overview.html) JavaScript est disponible si vous avez besoin d’informations supplémentaires sur [!DNL AppMeasurement.js]JavaScript.) |
| Synchronisation de l’ID client pour Analytics | See [Analytics - synching the customer ID](../core-services/core-services.md#section_AD473A6A21C1446498E700363F9A8437) (below). |
...
In Adobe Target, the `mbox3rdpartyid` needs to get the customer ID and send it to [!DNL Target]. (voir [Utilisation des attributs](https://docs.adobe.com/content/help/en/target/using/audiences/visitor-profiles/working-with-customer-attributes.html) du client dans [!DNL Target]).

Chaque fois qu’un visiteur s’authentifie sur votre site ou s’identifie d’une autre manière, votre mise en œuvre doit afficher l’ID client CRM sur la page dans l’application. Par la suite, vous pouvez utiliser l’appel de fonction approprié pour synchroniser votre ID client avec Experience Cloud. Cette synchronisation stocke l’ID de client CRM  dans Experience Cloud et active les attributs de ce client pour une utilisation dans Experience Cloud.
...
* Renseignez la *`Customer ID (52mc210tr42)`* variable dans une prop ou une eVar.
...
Les données d’attribut du client sont rendues disponibles après la connexion des. Si vous n’avez pas encore mis en oeuvre le dernier service d’ID d’Experience Cloud et si vous avez toujours suivi les ID de client dans une prop ou une eVar, vous pouvez demander un processus qui envoie des connexions historiques à Experience Cloud. Ce processus vous permet de commencer à utiliser immédiatement les attributs du client.
...
Experience Cloud services (such as Experience Cloud ID Service and the [!UICONTROL People service]) are associated with an Experience Cloud organization instead of an individual Analytics report suite. Pour garantir le bon fonctionnement de ces services, chaque suite de rapports Analytics doit être mappée à une organisation Experience Cloud.
...
In [!UICONTROL Dynamic Tag Management], click **[!UICONTROL <Web Property Name>]**>**[!UICONTROL  Aperçu ]**>**[!UICONTROL  Ajouter un outil ]**>**[!UICONTROL  Adobe Analytics ]**. Voir Paramètres[d’](https://docs.adobe.com/content/help/en/dtm/using/tools/analytics-dtm.html)Adobe Analytics dans la gestion dynamique des balises pour en savoir plus sur le déploiement.

## Etape 5. (Adobe Target) Update your Adobe Target implementation {#section_C2F4493C7A36406DAE2266B429A4BD24}

* Il est recommandé d’ajouter un [Adobe ](https://docs.adobe.com/content/help/en/launch/using/extensions-ref/adobe-extension/targetv2-extension/adobe-target-extension-v2.html) dans [!UICONTROL Experience Platform Launch], de sorte que la récupération de votre bibliothèque soit automatique. Vous pouvez également configurer l’extension [du service](https://docs.adobe.com/content/help/en/launch/using/extensions-ref/adobe-extension/id-service-extension/overview.html) Experience Cloud ID pour les  Adobe (et d’autres solutions) à l’aide du lancement [!UICONTROL de la plateforme]d’expérience. La mise à jour du service [!UICONTROL d’ID] Experience Cloud **est requise** pour que les  Adobe utilisent les services principaux. (Si vous utilisez la gestion [!UICONTROL dynamique des balises], ajoutez un outil [de  de](https://docs.adobe.com/content/help/en/dtm/using/tools/target.html)Adobe. Vous pouvez également utiliser la gestion [!UICONTROL dynamique des balises] pour déployer le service Experience Cloud ID pour les  Adobe.)
...
Procédez comme suit pour vous assurer que le service Experience Cloud ID est correctement mis en oeuvre sur votre site.

1. Effacez les cookies de votre site afin que vous puissiez voir la requête au service d’ID d’Experience Cloud (la requête survient lors de la première visite, puis environ une fois par et par semaine).
...
1. Verify that the Analytics request contains the `mid` parameter (the Experience Cloud ID). Pendant la période de grâce (si elle est activée), vous devriez également voir un `aid` paramètre (l’identifiant de Analytics).

Réponse attendue contenant l’ID Experience Cloud :

![](assets/mac_id_response.png)

Demande d’image Analytics contenant l’ID Experience Cloud (également appelé `mid` ID _de_) :
...
Une fois que vous avez déployé le service Experience Cloud ID, les nouveaux ne reçoivent plus d’Analytics Experience Cloud ID de votre serveur de collecte de données. Si des sections de votre site n’ont pas encore mis en oeuvre le service d’ID d’Experience Cloud, lorsque les accèdent à ces sections, l’ID d’Experience Cloud n’est pas reconnu et un ID de Analytics hérité est affecté aux. Cela peut entraîner des problèmes potentiels, notamment des visites  et une attribution incorrecte.
...
Le déploiement du service d’ID d’Experience Cloud sur les sites qui utilisent plusieurs fichiers JavaScript ou d’autres technologies (telles que Flash) peut entraîner des problèmes de coordination car vous devez activer le service d’ID d’Experience Cloud simultanément sur toutes les parties de votre site. En configurant une période de grâce, les nouveaux continuent de recevoir un ID de Analytics du service d’ID, de sorte que les puissent être identifiés de manière cohérente sur les sections de votre site qui n’ont pas été mises à niveau pour utiliser le service d’ID de.
...
Les utilisateurs ajoutés au groupe Attributs [!UICONTROL du] client verront l’option de menu Attributs [!UICONTROL du] client sur le côté gauche de l’interface d’Experience Cloud.
...
### [!UICONTROL Personnes] > [!UICONTROL  de la bibliothèque]
...
##  de données et  de confidentialité
...
* Caractéristiques et segments pour lesquels les remplissent les conditions requises en fonction des règles configurées dans  Gestionnaire de  de.
* (Facultatif) Un ou plusieurs de vos identifiants. Selon votre implémentation du service d’ID, vous pouvez également envoyer un ou plusieurs de vos ID, tels que des ID de gestion de la relation client ou des adresses électroniques hachées. Si ces données sont envoyées à Adobe [!DNL Analytics], elles sont transférées à la gestion de l’audience d’Adobe. Adobe recommande de ne pas fournir de données personnelles à Adobe [!DNL Analytics]. Utilisez plutôt un hachage unidirectionnel pour masquer les données avant de les envoyer à Adobe.
...
Tous ces éléments de données sont transmis à Adobe Audience Manager sous forme de fichiers journaux. Audience Manager traite et stocke ces données aux États-Unis.  Gestionnaire  de ne propose pas d’option permettant de stocker ou de traiter ces données en dehors des Etats-Unis.
...
Voir CNAME de collecte de [données et suivi](https://docs.adobe.com/content/help/en/id-service/using/reference/analytics-reference/cname.html) inter-domaines pour activer le suivi inter-domaines.
